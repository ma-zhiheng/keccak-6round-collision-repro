...
static constexpr int BASIS_SIZE = static_cast<int>(core2_data::BASIS_SIZE);
...
__global__ void search_kernel(
    ...
    for (std::uint64_t i = tid; i < count; i += stride) {
        ...
        if (alpha4 && digest && atomicCAS(&totals->found_written, 0, 1) == 0) {
...
void reconstruct_cuda_sample(
    ...
    m1 = core2_data::PARTICULAR_MESSAGE;
    ...
    for (int basis = 0; basis < BASIS_SIZE; ++basis) {
        ...
        for (int lane = 0; lane < LANES; ++lane) {
            m1[lane] ^= core2_data::BASIS_MESSAGE[static_cast<std::size_t>(basis)][lane];
        }
    }

    for (int lane = 0; lane < LANES; ++lane) {
        m2[lane] = m1[lane] ^ core2_data::ALPHA0[lane];
    }
}
...
HostEvalResult h_evaluate_full(const HostState& m1, const HostState& m2, int digest_bits) {
    ...
    result.alpha3 = h_diff_equal(s1, s2, core2_data::ALPHA3);
    if (!result.alpha3) {
        return result;
    }
    ...
    h_round(s1, 3);
    h_round(s2, 3);
    result.alpha4 = h_diff_equal(s1, s2, core2_data::ALPHA4);
    if (!result.alpha4) {
        return result;
    }
...
void copy_constants_to_device() {
    check_cuda(cudaMemcpyToSymbol(D_PARTICULAR, core2_data::PARTICULAR_MESSAGE.data(), sizeof(std::uint64_t) * LANES), "copy particular");
    check_cuda(cudaMemcpyToSymbol(D_ALPHA0, core2_data::ALPHA0.data(), sizeof(std::uint64_t) * LANES), "copy alpha0");
    check_cuda(cudaMemcpyToSymbol(D_ALPHA2, core2_data::ALPHA2.data(), sizeof(std::uint64_t) * LANES), "copy alpha2");
    check_cuda(cudaMemcpyToSymbol(D_ALPHA3, core2_data::ALPHA3.data(), sizeof(std::uint64_t) * LANES), "copy alpha3");
    check_cuda(cudaMemcpyToSymbol(D_ALPHA4, core2_data::ALPHA4.data(), sizeof(std::uint64_t) * LANES), "copy alpha4");
    ...
    check_cuda(
        cudaMemcpyToSymbol(D_BASIS, &core2_data::BASIS_MESSAGE[0][0], sizeof(std::uint64_t) * BASIS_SIZE * LANES),
        "copy basis"
    );
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "CUDA Table 7 core No. 2 trail search\n";
    ...
    for (std::size_t i = 0; i < results.size(); ++i) {
        ...
        if (result.has_found) {
            ...
            label << "FOUND alpha4/digest candidate sample=" << result.found_sample << " device=" << devices[i];
            std::cout << "  " << label.str() << "\n";
        ...
        if (options.alpha3_only) {
            for (std::uint64_t sample_number : result.alpha3_samples) {
                ...
                if (eval.alpha4 && eval.digest) {
                    std::ostringstream label;
                    label << "FOUND alpha4/digest candidate sample=" << sample_number << " device=" << devices[i];
                    std::cout << "  " << label.str() << "\n";
